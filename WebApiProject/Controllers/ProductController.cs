...
namespace TestWebApplication.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("")]
        public async Task<IActionResult> GetAllProductsAsync([FromQuery] string searchText = "", [FromQuery] int pageIndex = 0, [FromQuery] int pageSize = 30)
        {
            return Ok(ApiResult<List<ProductDto>>.Success(await _productService.GetAllProductsAsync(searchText, pageIndex, pageSize)));
        }
        [HttpGet]
        [Route("{productId}")]
